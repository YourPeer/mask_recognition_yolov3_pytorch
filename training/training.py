...
def train(config):
    ...
    for k,epoch in enumerate(range(config["epochs"])):
...
#                                                            config["global_step"])
                ...
                # net.train(True)
        _save_checkpoint(k,net.state_dict(), config)
        lr_scheduler.step()
...
def _save_checkpoint(k,state_dict, config, evaluate_func=None):
    # global best_eval_result
    checkpoint_path = os.path.join(config["working_dir"], "model_"+str(k)+".pth")
...
def main():
    ...
    if len(sys.argv) != 2:  #参数数量错误
        logging.error("Usage: python training.py params.py")
        sys.exit()
    params_path = sys.argv[1]
    if not os.path.isfile(params_path):   #配置文件
        logging.error("no params file found! path: {}".format(params_path))
        sys.exit()
    config = importlib.import_module(params_path[:-3]).TRAINING_PARAMS
    config["batch_size"] *= len(config["parallels"])  #内容错误报错在这，这里每一个gpu都有一个batch_size

    # sub_working_dir 用来存放生成的模型pth
...
#    logging.info("sub working dir: %s" % config["sub_working_dir"])
...
#    logging.info("Please using 'python -m tensorboard.main --logdir={}'".format(sub_working_dir))
    ...
    # Start training
    os.environ["CUDA_VISIBLE_DEVICES"] = ','.join(map(str, config["parallels"]))#设置gpu
    train(config)#开始训练
